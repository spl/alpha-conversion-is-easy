...
-- A structure for the class of operations that give a fresh element, an
-- element that is not a member of the given finite set.
--
...
structure has_fresh [class] (A : Type) :=
...
definition fresh_nat (X : finset ℕ) : Σ x, x ∉ X :=
  ...
  sigma.mk (succ (Max₀ X)) fresh_not_mem

-- The `has_fresh` instance for `nat`
definition has_fresh_nat [instance] : has_fresh ℕ :=
  has_fresh.mk fresh_nat
