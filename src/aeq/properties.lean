...
, exp.subst.apply (exp.subst.update (fresh X).1 (fresh Y).1 F) (exp.insert_var (fresh X).1 e)
  ...
  | X Y F (exp.var x) :=
    begin
      ...
      rw [exp.subst.update.ne F (vname.insert (fresh X).1 x) h, vname.eq_of_erase_insert x h]
    end
  | X Y F (exp.app f e) :=
...
: aeq.extend
  ...
  begin
    ...
    by_cases h : x₁.1 = a,
    { /- h : x₁.1 = a -/
      rw [exp.subst.update.eq x₁ h, exp.subst.update.eq x₁ h],
      rw [exp.subst.apply, exp.subst.update.eq_self]
    },
    { /- h : x₁.1 ≠ a -/
      calc
        ...
        exp.subst.apply (exp.subst.update b' c' G) (exp.insert_var b' (F (vname.erase x₁ h)))
            : by rw exp.subst.update.ne F x₁ h
        ...
        exp.subst.update a c' (exp.subst.apply G ∘ F) x₁
            : by rw ←exp.subst.update.ne (exp.subst.apply G ∘ F) x₁ h
    }
  end
