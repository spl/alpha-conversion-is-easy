...
(r : ∀ {Y : vs V} (F : exp.subst (insert a X) Y),
  exp.subst.apply (exp.subst.update_var ((fresh (insert a X)).1) (fresh Y).1 F) (exp.insert_var ((fresh (insert a X)).1) e)
    ≡α⟨vrel.id (insert (fresh Y).1 Y)⟩
  exp.insert_var (fresh Y).1 (exp.subst.apply F e))
: exp.subst.apply
      (exp.subst.update_var a (fresh (insert (fresh Y).1 Y)).1 (exp.subst.update_var (fresh X).1 (fresh Y).1 F))
  ...
  exp.map
      ...
      (exp.insert_var (fresh Y).1 (exp.subst.apply (exp.subst.update_var a (fresh Y).1 F) e))
...
(r : ∀ {Y : vs V} (F : exp.subst (insert a X) Y),
  exp.subst.apply (exp.subst.update_var ((fresh (insert a X)).1) (fresh Y).1 F) (exp.insert_var ((fresh (insert a X)).1) e)
    ≡α⟨vrel.id (insert (fresh Y).1 Y)⟩
  exp.insert_var (fresh Y).1 (exp.subst.apply F e))
: exp.subst.apply (exp.subst.update_var (fresh X).1 (fresh Y).1 F) (exp.insert_var (fresh X).1 (exp.lam e))
    ≡α⟨vrel.id (insert (fresh Y).1 Y)⟩
  exp.insert_var (fresh Y).1 (exp.subst.apply F (exp.lam e)) :=
  begin
    rw [exp.insert_var.of_lam₂ e],
    have h : exp.subst.apply (exp.subst.update_var (fresh X).1 (fresh Y).1 F) (exp.lam (exp.map (vset.prop_subset_of_insert_comm (fresh X).1 a X) (exp.insert_var (fresh X).1 e)))
           = exp.lam (exp.subst.apply (exp.subst.update_var a (fresh (insert (fresh Y).1 Y)).1 (exp.subst.update_var (fresh X).1 (fresh Y).1 F)) (exp.map (vset.prop_subset_of_insert_comm (fresh X).1 a X) (exp.insert_var (fresh X).1 e))) :=
    ...
    rw [exp.insert_var.of_lam₂ (exp.subst.apply (exp.subst.update_var a (fresh Y).1 F) e)],
    apply aeq.lam,
