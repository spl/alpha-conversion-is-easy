...
theorem update_var_of_ne (a b : V) (F : subst X Y) (x : ν∈ insert a X) (p : x.1 ≠ a)
: subst.update_var a b F x = insert_var b (F (vname.erase x p)) :=
  by simp [exp.subst.update_var, exp.subst.update, dif_neg p]
