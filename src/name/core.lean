...
@[reducible]
protected
definition rw : X = Y → ν∈ X → ν∈ Y :=
  λ H x, ⟨x.1, rw_constraint H x.2⟩

@[reducible]
protected
definition eq {x₁ x₂ : ν∈ X} (h : x₁.1 = x₂.1) : x₁ = x₂ :=
  psigma.eq h rfl

@[reducible]
definition self (a : V) (X : finset V) : ν∈ finset.insert a X :=
  ⟨a, self_constraint⟩

-- Erase a variable from the free variable set if it is not equal to this
-- variable.
@[reducible]
definition erase (x : ν∈ finset.insert a X) : x.1 ≠ a → ν∈ X :=
  λ x_ne_a, ⟨x.1, erase_constraint x.2 x_ne_a⟩

-- Insert a variable name into the free variable set.
@[reducible]
definition insert (a : V) (x : ν∈ X) : ν∈ finset.insert a X :=
